(function(serverAddress) {
  ...
  _ = require('lodash');
  util = require('util');
  debug = require('debug')('keti.onem2m');
  ...
  CSE) on this hop, omitting any leading
    ...
    function mqttMessageHandler(topic, message) {
      ...
      var topicFrom = topicArray[3];
      var topicTo = topicArray[4];
      var topicParams = topicArray.splice(5);
      ...
      if( handler ) {
        ...
        var handled = handler(thisObj.mqttServerAddress, topicTo, topicParams, notiEventType, primitiveContent);
        ...
        if(handled) {
          ...
          var respTopic = "/oneM2M/resp/" + topicFrom + "/" + topicTo + "/json";
  ...
  function CreateResource(parentResourceUrl, resource, origin) {
    ...
    return new Promise(function(resolved, rejected) {
      ...
      Http(options)
        ...
        .catch(function(error) {
          debug( 'fail to create resource ', error );
          rejected(error);
        });
    });
  }
