...
.directive('smartwizard', smartWizardFormDirective)
.directive('onKeyEnter', onKeyEnterDirective)
;
...
//Wizard form
function smartWizardFormDirective() {
  var directive = {
    restrict: 'A',
    link: link,
    transclude: false
  ...
  function link(scope, element, attrs) {
    ...
    scope.stepIndexList     = $(element).find('.step-anchor .nav-item');
    scope.stepContentList   = $(element).find('.sw-container .step-content');

    scope.btnPrev     = $(element).find('.sw-btn-prev');
    scope.btnNext     = $(element).find('.sw-btn-next');
    scope.btnFinish   = $(element).find('.sw-btn-finish');

    scope.updateButtons = function() {
      if(scope.currentStep == 0)
        scope.btnPrev.prop( 'disabled', true );
      else
        scope.btnPrev.prop( 'disabled', false );

      if(scope.currentStep == (scope.stepIndexList.length-1)) {
        scope.btnNext.css('display', 'none');
        scope.btnFinish.css('display', 'inline-block');
      }
      else{
        scope.btnNext.css( 'display', 'inline-block' );
        scope.btnFinish.css( 'display', 'none');
      }
    }
    ...
    scope.showStep = function(step) {
      ...
      var newStep = Math.max(0, Math.min(scope.stepIndexList.length-1, step));
      ...
      if(oldStep != -1) {
        $(scope.stepIndexList[oldStep]).addClass('done');
        $(scope.stepIndexList[oldStep]).removeClass('active');

        $(scope.stepContentList[oldStep]).hide();
      }

      $(scope.stepIndexList[scope.currentStep]).addClass('active');
      $(scope.stepIndexList[scope.currentStep]).removeClass('done');

      $(scope.stepContentList[scope.currentStep]).show();
